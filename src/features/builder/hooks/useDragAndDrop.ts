...
export const useDragAndDrop = () => {
  ...
  const createDefaultStyles = useCallback((type: keyof typeof TIPOS_BLOQUES): BlockStyles => {
    const baseStyles = {
      [dispositivoActual]: {
        margin: "8px",
        padding: "8px",
  ...
  const createDefaultConfig = useCallback((type: keyof typeof TIPOS_BLOQUES): BlockConfig => {
    const baseConfig = {
      isVisible: true,
      isDisabled: false,
    };
    ...
    return {
      blockId: "", // Se asignará después
      config: typeSpecificConfig[type] || baseConfig,
    };
  }, []);
  ...
  const handleDrop = useCallback((
    ...
    try {
      ...
      if (targetContainerId) {
        ...
        console.log(`Agregando ${dragData.type} al contenedor ${targetContainerId}`);
      }

      // Agregar el bloque al builder
      addBlock(newBlock, defaultStyles, defaultConfig);

      console.log(`✅ Elemento ${dragData.label} agregado con ID: ${newBlockId}`);
    } catch (error) {
